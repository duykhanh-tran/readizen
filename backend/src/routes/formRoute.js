...
router.post("/submit", verifyToken, createForm);          // Khách hàng gửi form lên
router.get("/my", verifyToken, getMyForms);               // Khách hàng lấy danh sách form của mình
router.get("/all", verifyToken, getAllForms);            // Admin lấy danh sách form về xem
router.put("/:id/status", verifyToken, updateFormStatus); // Admin cập nhật tiến độ xử lý form
